...
let bounds = Leaflet.latLngBounds(maxBounds1, maxBounds2);
let run;
...
export class mapEditorClass {
    ...
    initializeMap(objRef) {
        ...
        layerGroup = Leaflet.layerGroup().addTo(editMap);
    ...
    addNewStage(e) {
        if (run.stages.length < 1) {
            ...
            let tempMarker = Leaflet.circleMarker(e.latlng, { fillOpacity: 1 }).addTo(layerGroup);
            }
        else {
